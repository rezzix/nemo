...
public class TimeReportController {
    ...
    public TimeReportController(TimeLogRepository timeLogRepository, UserRepository userRepository,
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> timeByProject(
        ...
        Map<Long, BigDecimal> byProject = logs.stream()
                .collect(Collectors.groupingBy(
                        tl -> tl.getTask().getProject().getId(),
    ...
    @GetMapping("/time-by-task")
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER', 'EXECUTIVE', 'HR')")
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> timeByTask(
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate,
        ...
        Map<Long, BigDecimal> byTask = logs.stream()
                .collect(Collectors.groupingBy(
                        tl -> tl.getTask().getId(),
                        Collectors.reducing(BigDecimal.ZERO, TimeLog::getHours, BigDecimal::add)
                ));

        List<Map<String, Object>> result = byTask.entrySet().stream()
                .map(e -> Map.of("taskId", (Object) e.getKey(), "totalHours", e.getValue()))
                .toList();
