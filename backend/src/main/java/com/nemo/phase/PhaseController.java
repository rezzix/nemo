...
public class PhaseController {
    ...
    public ResponseEntity<ApiResponse<List<PhaseDto>>> list(
        ...
        if ("OPEN".equalsIgnoreCase(status)) {
            phases = phaseService.getOpenByProjectId(projectId);
        } else if ("CLOSED".equalsIgnoreCase(status)) {
            phases = phaseService.getByProjectIdAndStatus(projectId, Phase.PhaseStatus.CLOSED);
        } else {
            phases = phaseService.getByProjectId(projectId);
        }
        List<PhaseDto> dtos = phaseService.enrichWithComputedFields(phaseMapper.toDtoList(phases));
