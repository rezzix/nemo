...
Backend unit, integration, and E2E tests for the `identity` group (`company/`, `user/`), compiled from the legacy `docs/test-plan.md`. The backend currently ships **no** test files; this is the planned coverage. Shared infrastructure and the global strategy live in [the test plan](/testing/test-plan.md).
...
Scenarios to add to `postman/nemo-api-collection.json` (see [Postman collection](/testing/postman-collection.md)):
...
- [Identity module entities](/modules/identity/module-entities.md) — entities under test.
...
- [Multi-tenancy](/security/multi-tenancy.md) — the `company_id` scoping asserted by E2E-21..23.