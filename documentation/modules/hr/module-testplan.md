...
- **Integration — endpoints**: leave request CRUD at `/api/leave-requests`, entitlement endpoints, plus RBAC variations (HR/MANAGER/ADMIN vs self) and validation, following the pattern of the [identity](/modules/identity/module-testplan.md) and [delivery](/modules/delivery/module-testplan.md) plans.
- **Manual E2E**: request leave, approve/reject as HR/MANAGER, verify balance updates; (planned) create an evaluation cycle.
- **Evaluation** is itself a [planned stub](/modules/hr/evaluation-module.md) with no backend package yet.

## Cross-references

- [HR module entities](/modules/hr/module-entities.md) — entities to test.
- [Test plan](/testing/test-plan.md) — global strategy, infrastructure, and test data.
- [Postman collection](/testing/postman-collection.md) — extend the collection with leave endpoints.