...
- **Unit — services**: `ClientService` (client + contact CRUD), `ProjectExpenseService`, `ProjectPaymentService` (payment reconciliation with `BankTransaction` — see [reconciliation module](/modules/finance/reconciliation-module.md)).
- **Integration — endpoints**: client/contact CRUD at `/api/clients`, expense/payment endpoints, plus RBAC variations (ADMIN/MANAGER/CONTRIBUTOR) and validation (duplicate, key length, missing required fields), following the pattern of the [identity](/modules/identity/module-testplan.md) and [delivery](/modules/delivery/module-testplan.md) plans.
- **Manual E2E**: create client, add contact, log expense, record payment and reconcile against a bank transaction.

## Cross-references

- [Commercial module entities](/modules/commercial/module-entities.md) — entities to test.
- [Test plan](/testing/test-plan.md) — global strategy, infrastructure, and test data.
- [Postman collection](/testing/postman-collection.md) — extend the collection with commercial endpoints.