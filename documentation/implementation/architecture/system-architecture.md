...
See [backend layering](/overview/backend-layering.md) for the per-module pattern, [monorepo structure](/overview/monorepo-structure.md) for the package layout, and the [modules index](/modules/index.md) for each domain package.

## Frontend architecture

The SPA follows a layered architecture (Pages → Components → Stores → API Layer → WebSocket Hook) built with React 19, Zustand 5, Tailwind CSS v4, and React Router 7. The full structural view, tech stack, routing/guards, API client layer, state management, and dashboard details live in [frontend architecture](/implementation/architecture/frontend-architecture.md).
...
The full role × action permissions matrix lives in [features and access](/security/features-and-access.md); role meanings and `AuthHelper` checks are in [authorization (RBAC)](/security/authorization-rbac.md). The login/session sequence diagram is in [UML diagrams](/implementation/architecture/uml-diagrams.md).
...
When a task moves (status change), the backend publishes to `/topic/kanban/{projectId}`; all subscribed clients update their local board. The board component uses a `useKanbanWebSocket` hook for the STOMP connection. See [WebSockets](/implementation/cross-cutting/websockets.md) and the Kanban sequence in [UML diagrams](/implementation/architecture/uml-diagrams.md).
...
`StorageService` abstracts file operations; the default writes to a configurable local directory, swappable to S3 by implementing the same interface. Attachments link to tasks/wiki via the `Attachment` entity. See [file storage](/implementation/cross-cutting/file-storage.md).
...
See [request data flow](/overview/request-data-flow.md) for the full lifecycle and error response format.
...
See [Nemo system](/overview/nemo-system.md) for the deployment model and run modes.
...
See [REST conventions](/implementation/api/rest-conventions.md) and [error handling](/implementation/api/error-handling.md).
...
See [database schema](/overview/database-schema.md) for the full naming table.

## Cross-references

- [UML diagrams](/implementation/architecture/uml-diagrams.md) — the diagrammatic views (class, use-case, state, sequence, component).
- [Overview](/overview/index.md) — system shape, layering, request flow, tech stack.
- [Modules index](/modules/index.md) — the domain packages this architecture is built from.
- [Security index](/security/index.md) — authentication, RBAC, multi-tenancy, access matrix.