...
Business exceptions (`EntityNotFoundException`, `AccessDeniedException`, …) map to the appropriate HTTP status codes. Validation errors come from Spring Validator annotations on DTOs. See [error handling](/implementation/api/error-handling.md).

## Cross-references

- [Backend layering](/overview/backend-layering.md) — the layers the request crosses.
- [Security: authentication](/security/authentication.md) — the session model.
- [Error handling](/implementation/api/error-handling.md) — the exception-to-status mapping.
- [Cross-cutting: audit logging](/implementation/cross-cutting/audit-logging.md) — the `@Audited` capture step.