...
- **API testing** — exercised via the [Postman/Newman collection](/state/postman-collection.md).
- **Frontend** — ESLint (`npm run lint`) and `tsc -b` type-check during `npm run build`; component/unit tests are planned (Vitest), see the [cross-cutting module test plan](/state/test-plans/cross-cutting/module-testplan.md).
...
Each module group has a `module-testplan.md` under [State](/state/test-plans/index.md) with the unit, integration, frontend, and E2E cases for its submodules, compiled from the legacy plan:

* [Cross-cutting module test plan](/state/test-plans/cross-cutting/module-testplan.md) — auth, captcha, RBAC, org config, task types/statuses, audit, error handling, frontend infra.
* [Identity module test plan](/state/test-plans/identity/module-testplan.md) — user and company endpoints.
* [Delivery module test plan](/state/test-plans/delivery/module-testplan.md) — program, project, task, sprint, PMO/RAID.
* [Commercial module test plan](/state/test-plans/commercial/module-testplan.md) — *planned* (client, expense, payment).
* [Finance module test plan](/state/test-plans/finance/module-testplan.md) — *planned* (banks, reconciliation).
* [HR module test plan](/state/test-plans/hr/module-testplan.md) — *planned* (leave, evaluation).
* [Time tracking module test plan](/state/test-plans/timetracking/module-testplan.md) — time logs, timesheets, reports.
* [Content module test plan](/state/test-plans/content/module-testplan.md) — wiki, attachment.

## Cross-references

- [Postman collection](/state/postman-collection.md) — the API test asset.
- [Tech stack](/charter/overview/tech-stack.md) — lint/build tooling.
- [Backend layering](/charter/overview/backend-layering.md) — the Controller/Service/Repository tiers under test.