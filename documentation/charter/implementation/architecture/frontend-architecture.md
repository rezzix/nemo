...
A catch-all redirects unknown routes to `/`. Role gating aligns with the backend [RBAC model](/charter/security/authorization-rbac.md).

Role gating aligns with the backend [RBAC model](/charter/security/authorization-rbac.md); the guard hierarchy (GuestGuard → AuthGuard → RoleGuard/AdminGuard) is described in the Routing and guards section above.
...
The role-to-dashboard mapping follows the [features and access](/charter/security/features-and-access.md) matrix.
